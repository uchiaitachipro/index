...
if __name__ == "__main__":
    ...
    # 加载模型
    model_path = Path("./noise_detector_model_v4.pkl")
    ...
    # 验证所有文件
    correct_noise = 0  # 正确识别为有杂音
    correct_clean = 0   # 正确识别为无杂音
    false_positive = []  # 误识别为有杂音（正常文件被误识别）
    false_negative = []  # 误识别为无杂音（杂音文件被漏检）
        ...
        try:
            ...
            if is_noise:
                if prediction == 1:
                    ...
                    false_negative.append((file_name, probability))
            else:
                if prediction == 0:
                    correct_clean += 1
                else:
                    false_positive.append((file_name, probability))
    ...
    print(f"有杂音文件数: {len(noise_files)}")
    print(f"无杂音文件数: {len(wav_files) - len(noise_files)}")
    print()
    print(f"✓ 正确识别为有杂音: {correct_noise} / {len(noise_files)}")
    print(f"✓ 正确识别为无杂音: {correct_clean} / {len(wav_files) - len(noise_files)}")
    print()
    
    if false_positive:
        print(f"✗ 误识别为有杂音（正常文件被误识别）: {len(false_positive)} 个")
        for name, prob in false_positive[:10]:
            print(f"  - {name} (概率: {prob:.4f})")
        if len(false_positive) > 10:
            print(f"  ... 还有 {len(false_positive) - 10} 个")
    else:
        print("✓ 没有正常文件被误识别为有杂音")
    
    if false_negative:
        print(f"\n✗ 误识别为无杂音（杂音文件被漏检）: {len(false_negative)} 个")
        for name, prob in false_negative:
            print(f"  - {name} (概率: {prob:.4f})")
    else:
        print("\n✓ 所有杂音文件都被正确识别")
    
    accuracy = (correct_noise + correct_clean) / len(wav_files)
