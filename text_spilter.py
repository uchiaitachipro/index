...
_tokenizer_cache: Optional[TextTokenizer] = None
_tokenizer_vocab_file: Optional[str] = None


def split_text_by_characters(
    text: str,
    max_chars_per_segment: int,
    vocab_file: str = "checkpoints/bpe.model",
    max_text_tokens_per_segment: int = 120,
    quick_streaming_tokens: int = 0
) -> List[str]:
    """
    将文本按特定字符分割，可以指定最大字符数
    
    直接复用 TextTokenizer 的分割逻辑，然后将 tokens 转换回文本。
    参考 infer_v2_subtitle.py (486-497) 的实现方式。
    
    Args:
        text: 要分割的文本
        max_chars_per_segment: 每个段落的最大字符数
        vocab_file: BPE 模型文件路径，默认为 "checkpoints/bpe.model"
        max_text_tokens_per_segment: 每个段落的最大 token 数（用于初始分割）
        quick_streaming_tokens: 快速流式处理的 token 数阈值
    
    Returns:
        分割后的文本段落列表（每个段落不超过 max_chars_per_segment 字符）
    """
    global _tokenizer_cache, _tokenizer_vocab_file
    
    if len(text) == 0:
        return []
    
    # 初始化或复用 tokenizer
    if _tokenizer_cache is None or _tokenizer_vocab_file != vocab_file:
        text_normalizer = TextNormalizer()
        _tokenizer_cache = TextTokenizer(
            vocab_file=vocab_file,
            normalizer=text_normalizer,
        )
        _tokenizer_vocab_file = vocab_file
    ...
    for segment_tokens in segments:
        segment_ids = tokenizer.convert_tokens_to_ids(segment_tokens)
        segment_text = tokenizer.decode(segment_ids)
        original_text_segments.append(segment_text)
    
    # 检查字符数限制，如果超过则进一步分割
    final_segments = []
    for segment_text in original_text_segments:
        if len(segment_text) <= max_chars_per_segment:
            final_segments.append(segment_text)
        else:
            # 如果超过字符数限制，按字符数强制分割
            for i in range(0, len(segment_text), max_chars_per_segment):
                final_segments.append(segment_text[i:i + max_chars_per_segment])
...
if __name__ == "__main__":
    ...
    for i, text in enumerate(test_cases):
        print(f"测试 {i+1}:")
        print(f"原始文本: {text}")
        segments = split_text_by_characters(
            text, 
            max_chars_per_segment=max_chars
        )
        print(f"分割结果 ({len(segments)} 段):")
        for j, segment in enumerate(segments):
            print(f"  {j+1}. [{len(segment)} 字符] {segment}")
            if len(segment) > max_chars:
                print(f"    警告: 段落 {j+1} 超过最大字符数限制!")
        print("-" * 50)
