...
def extract_features_v4_3(y: np.ndarray, sr: int, tail_ms=200, ref_ms=300, hi_band=(5000, 15000)):
    """
    提取音频特征（V4.3版本，在V4.2基础上增强）
    ...
    # 特征1: 尾部最后2ms的能量（极短时间检测，检测非常短的杂音）
...
def prepare_training_data(data_dir: Path, noise_files: set):
    ...
    for wav_file in wav_files:
        try:
            ...
            features = extract_features_v4_3(audio_data, sr)
            X.append(features)
...
if __name__ == "__main__":
    ...
    models_to_try = [
        {
            'name': 'RandomForest_V4.3_超深度优化',
            'model': RandomForestClassifier(
                n_estimators=3500,  # 更多树
    ...
    if hasattr(best_model, 'feature_importances_'):
        ...
        print("\n特征重要性 (Top 25):")
        for i in range(min(25, len(indices))):
    ...
    with open('./noise_detector_features_v4_3.json', 'w') as f:
        json.dump(feature_info, f, indent=2, ensure_ascii=False)
    print(f"特征信息已保存到: noise_detector_features_v4_3.json")
