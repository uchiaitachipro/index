...
if __name__ == "__main__":
    ...
    # 使用全部数据训练（追求100%准确率和更高的OOB分数）
    print("\n使用全部数据训练模型（不划分测试集）...")
    
    # 尝试多种模型配置，找到能达到最高OOB分数的配置
    models_to_try = [
        {
            'name': 'RandomForest_深度优化_更多树',
    ...
    for model_config in models_to_try:
        ...
        if train_score >= 0.9999 and oob_score > best_oob_score:
            best_model = model
            best_name = model_config['name']
            best_oob_score = oob_score
            best_train_score = train_score
            print(f"✓ 当前最佳模型！OOB分数: {oob_score:.6f}")
    ...
    with open('./noise_detector_features_v4_enhanced.json', 'w') as f:
        json.dump(feature_info, f, indent=2)
    print(f"特征信息已保存到: noise_detector_features_v4_enhanced.json")
