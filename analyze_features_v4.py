...
if __name__ == "__main__":
    ...
    print(f"有杂音的文件数量: {len(noise_files)}")
    ...
    for wav_file in wav_files:
        try:
            ...
            features, intermediate = extract_features(audio_data, sr)
            all_features.append(features)
            
            file_name = wav_file.stem
            label = 1 if file_name in noise_files else 0
            all_labels.append(label)
            all_intermediate_data.append(intermediate)
    ...
    noise_features = X[y == 1]
    clean_features = X[y == 0]
    
    print("\n有杂音样本的特征统计:")
    print(f"  均值范围: [{noise_features.mean(axis=0).min():.4f}, {noise_features.mean(axis=0).max():.4f}]")
    print(f"  标准差范围: [{noise_features.std(axis=0).min():.4f}, {noise_features.std(axis=0).max():.4f}]")
    
    print("\n无杂音样本的特征统计:")
    print(f"  均值范围: [{clean_features.mean(axis=0).min():.4f}, {clean_features.mean(axis=0).max():.4f}]")
    print(f"  标准差范围: [{clean_features.std(axis=0).min():.4f}, {clean_features.std(axis=0).max():.4f}]")
    
    # 计算特征差异
    feature_diff = np.abs(noise_features.mean(axis=0) - clean_features.mean(axis=0))
    feature_std_ratio = (noise_features.std(axis=0) + 1e-12) / (clean_features.std(axis=0) + 1e-12)
    
    # 加载特征名称
    with open('./noise_detector_features_v4.json', 'r') as f:
        feature_info = json.load(f)
    feature_names = feature_info['feature_names']
    
    print("\n特征差异分析 (Top 20):")
    diff_indices = np.argsort(feature_diff)[::-1]
    for i in range(min(20, len(diff_indices))):
        idx = diff_indices[i]
        noise_mean = noise_features[:, idx].mean()
        clean_mean = clean_features[:, idx].mean()
        print(f"  {i+1}. {feature_names[idx]}: 差异={feature_diff[idx]:.4f} "
              f"(有杂音={noise_mean:.4f}, 无杂音={clean_mean:.4f})")
    
    # 保存分析结果
    analysis_result = {
        'feature_names': feature_names,
        'noise_mean': noise_features.mean(axis=0).tolist(),
        'clean_mean': clean_features.mean(axis=0).tolist(),
        'noise_std': noise_features.std(axis=0).tolist(),
        'clean_std': clean_features.std(axis=0).tolist(),
        'feature_diff': feature_diff.tolist(),
        'feature_std_ratio': feature_std_ratio.tolist(),
    }
    
    with open('./feature_analysis_v4.json', 'w') as f:
        json.dump(analysis_result, f, indent=2)
    
    print("\n分析结果已保存到: feature_analysis_v4.json")
    print("\n分析完成！")
