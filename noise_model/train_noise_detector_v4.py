...
def extract_features(y: np.ndarray, sr: int, tail_ms=200, ref_ms=300, hi_band=(5000, 15000)):
    """
    提取音频特征（用于机器学习）
    与train_noise_detector.py中的extract_features函数相同
...
if __name__ == "__main__":
    ...
    models_to_try = [
        {
            'name': 'RandomForest_深度优化',
            'model': RandomForestClassifier(
                n_estimators=1000,  # 增加树的数量
                max_depth=None,    # 不限制深度，完全生长
                min_samples_split=2,
                min_samples_leaf=1,
                random_state=42,
                class_weight={0: 1.0, 1: 3.0},  # 给有杂音类别更高权重
                max_features='sqrt',
                bootstrap=True,
                oob_score=True
            )
    ...
    for model_config in models_to_try:
        ...
        print(f"全部数据准确率: {train_score:.4f}")
        ...
        if errors:
            print(f"错误分类的文件 ({len(errors)} 个):")
            for name, true_label, pred_label in errors[:10]:  # 只显示前10个
    ...
    with open('./noise_detector_features_v4.json', 'w') as f:
        json.dump(feature_info, f, indent=2)
    print(f"特征信息已保存到: noise_detector_features_v4.json")
